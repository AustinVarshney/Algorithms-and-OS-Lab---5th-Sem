...
void SchedulerWindow::setupVisualizationArea()
{
    ...
    ganttChart->setMinimumHeight(450); // Much larger minimum height for better visibility
    ...
    ganttScrollArea->setMinimumHeight(500); // Much larger to show full chart with queue
...
void SchedulerWindow::updateGanttChart()
{
    ...
    int chartHeight = algorithmCombo->currentIndex() == 0 ? 420 : 250; // Much larger height for queue visibility
    ...
    int axisY = chartHeight - 150; // Move axis much higher to leave more space for queue
...
void SchedulerWindow::drawQueueVisualization(QPainter& painter, int axisY)
{
    ...
    // Position queue with proper spacing from timeline
    int queueSectionY = axisY + 50; // More spacing from timeline for better visibility
    ...
    // Draw queue boxes with better visibility
    QStringList colors = {"#FF6B6B", "#4ECDC4", "#45B7D1", "#96CEB4", "#FECA57", "#FF9FF3", "#54A0FF"};
    
    int queueX = 50;
    int queueY = queueSectionY + 30; // More spacing for labels
    int boxWidth = 45;  // Larger boxes for better visibility
    int boxHeight = 35; // Taller boxes
    int boxSpacing = 15; // More spacing between boxes
